...
function getTableData($tableName, $page, $limit)
{
    ...
    $query = mysql_query('SELECT * FROM `'.$tableName.'` WHERE level=0 LIMIT '.$startRow.', '.$limit);
...
{
    $countTotalRow = mysql_query('SELECT COUNT(*) AS total FROM `'.$tableName.'` WHERE level=0 ');
