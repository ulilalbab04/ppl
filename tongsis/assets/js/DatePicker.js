...
var DatePicker = new Class({
	...
	create: function(dp){
		...
		if (dp.options.yearOrder == 'desc'){
			for (var y = dp.options.yearRange + 25; y > (dp.options.yearStart - 1); y--){
				yearSel.options[i] = new Option(y, y);
				if (dp.year == y) yearSel.options[i].selected = true;
				i++;
			}
		} else {
			for (var y = dp.options.yearStart; y < (dp.options.yearRange + 1); y++){
				yearSel.options[i] = new Option(y, y);
				if (dp.year == y) yearSel.options[i].selected = true;
				i++;
			}
	...
	formatValue: function(dp, year, month, day){
		...
		dateStr = dp.options.format.replace( /mm/i, month ).replace( /yyyy/i, year ).replace( /dd/i, day );
